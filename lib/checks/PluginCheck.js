...
function CheckImportType(owner, repo, client) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (readme === undefined)
                throw "error";
            var EncodedReadme = yield client.repos.getContents({
                owner: owner,
                repo: repo,
                path: readme.path
            });
            readme = js_base64_1.Base64.decode(EncodedReadme.data["content"]);
            if (readme === undefined)
                throw "error";
...
function CheckRoot(owner, repo, client) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            var RootContents = yield client.repos.getContents({
                owner: owner,
                repo: repo,
                path: ""
            });
            RootContents.data.forEach(element => {
                if (valid_names.includes(element.name))
                    pluginExist = true;
            });
            if (pluginExist)
